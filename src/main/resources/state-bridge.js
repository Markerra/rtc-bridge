(() => {
    ...
    const hasCaptcha = () => {
        ...
        const captchaExists = frames.some((el) => {
            const str =
                `${el.className || ""} ${el.id || ""} ${el.src || ""}`.toLowerCase();
            return (
                visible(el) &&
        ...
        for (const frame of document.querySelectorAll("iframe")) {
            try {
                ...
                if (doc && doc.body) {
                    ...
                    if (
                        fText.includes("решить каптчу") ||
                        fText.includes("я не робот") ||
                        fText.includes("i'm not a robot")
                    ) {
                        return true;
                    }
                }
    ...
    const detect = () => {
        ...
        if (text.includes("разговор с nekto")) {
            return "CONNECTED";
        }
    ...
    const report = () => {
        try {
            ...
            if (currentState !== savedState) {
                console.log(
                    `[Bridge State] ${savedState || "NONE"} -> ${currentState}`,
                );
